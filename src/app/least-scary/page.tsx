...
import { Ghost, Smile, TrendingDown, ChevronRight, Loader2 } from 'lucide-react';
...
export default function LeastScaryPage() {
  const [entities, setEntities] = useState<EntityScore[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const fetchLeastScaryEntities = async () => {
      try {
        const response = await fetch('/api/entities/least-scary');
        if (!response.ok) {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading the least scary entities...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="text-center py-8 md:py-12 px-4">
        <div className="inline-flex items-center justify-center p-3 bg-green-100 rounded-2xl mb-4 md:mb-6">
          <Smile className="h-12 w-12 md:h-16 md:w-16 text-green-600" />
        </div>
        <h1 className="text-3xl md:text-5xl font-serif font-bold text-gray-900 mb-3 md:mb-4">The Least Scary Entities</h1>
        <p className="text-lg md:text-xl text-gray-700 max-w-2xl mx-auto">
          These are the most benign entities in our database. Perfect for those who prefer their entertainment without the frights!
        </p>
      </div>
      ...
      <div className="space-y-4 md:space-y-6 mb-8 md:mb-12 px-4 md:px-0">
        {entities.map((entity, index) => (
          ...
          >
            <div className="bg-white rounded-xl md:rounded-2xl shadow-sm border-2 border-gray-100 hover:border-green-200 hover:shadow-lg transition-all overflow-hidden">
                  ...
                  <div className="flex items-start gap-4 sm:gap-0 w-full sm:w-auto">
                    ...
                    <div className="flex-shrink-0">
                      <div className={`w-14 h-14 md:w-16 md:h-16 rounded-full flex items-center justify-center text-xl md:text-2xl font-bold text-white shadow-lg ${
                        ...
                        index === 1 ? 'bg-gradient-to-br from-green-500 to-green-700' :
                        index === 2 ? 'bg-gradient-to-br from-emerald-500 to-green-600' :
                  ...
                  <div className="flex-1 w-full">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-3">
                      <div className="flex-1">
                        <h2 className="text-xl md:text-2xl font-semibold text-gray-900 group-hover:text-green-600 transition-colors">
                          {entity.name}
                        </h2>
                    ...
                    <div className="flex flex-wrap gap-2 md:gap-3">
                      ...
                      <div className="flex items-center text-xs md:text-sm text-gray-500 group-hover:text-green-600 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row justify-center gap-3 sm:gap-4 mb-8 md:mb-12 px-4">
        ...
        >
          <TrendingDown className="h-4 w-4 md:h-5 md:w-5 rotate-180" />
          <span>View Scariest</span>
        </Link>
        <Link
          href="/"
